...
public class SimpleDataBusDao extends AbstractDaoImpl implements DataBusDao {
	...
	public PagingObject getRecords(ResourceConfiguration resourceConfiguration, Map<String, String> filters, Integer pageNum, Integer pageSize, String sortBy, Boolean reverse) {
		return new PagingObject(pageNum, pageSize, (List<Map<String, Object>>)getSimpleJdbcCall()
			.withProcedureName(resourceConfiguration.getStoredProcedure())
			.returningResultSet("results", new CamelCaseMapRowMapper())
			.execute(buildSqlParameterSource(resourceConfiguration, filters, pageNum, pageSize, sortBy, reverse))
			.get("results")
		);
	}
	...
	protected MapSqlParameterSource buildSqlParameterSource(ResourceConfiguration resourceConfiguration, Map<String, String> filters, Integer pageNum, Integer pageSize, String sortBy, Boolean reverse) {
		...
		if (resourceConfiguration.getFilters() != null) {
			for (Field field : resourceConfiguration.getFilters()) {
		...
		if (resourceConfiguration.isFilteringOnFieldsAllowed()) {
